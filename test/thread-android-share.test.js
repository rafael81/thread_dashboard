...
test("Threads Android share parser fix increments the app version consistently", () => {
  assert.match(manifestSource, /android:versionCode="10"/);
  assert.match(manifestSource, /android:versionName="0\.6\.4"/);
  assert.match(gradleSource, /versionCode = 10/);
  assert.match(gradleSource, /versionName = "0\.6\.4"/);
});
